...
/**
 * Script-specific instructions are useful for CJK text. For every other
 * natural language, let the naming model follow the user's actual wording
 * instead of guessing a language from its writing system.
 */
export type NamingLanguage = "Chinese" | "Japanese" | "Korean" | "UserLanguage";
...
export function detectDominantUserLanguage(parts: DialoguePart[]): NamingLanguage | undefined {
  const scores: Record<NamingLanguage, number> = { Chinese: 0, Japanese: 0, Korean: 0, UserLanguage: 0 };
  ...
  for (const part of parts) {
    ...
    if (!hasCjk) addScore("UserLanguage", scriptCount(text, /\p{L}/gu));
    seen += 1;
  }
...
export function getNamingLanguageInstruction(parts: DialoguePart[], fallbackLocale?: string): string {
  ...
  if (language === "UserLanguage") return "Write the label in the same dominant natural language used by the user. Do not translate it based on the Pi locale.";
...
/**
 * Returns the actual first user-authored turn. Unlike getFirstDialogue(), it
 * deliberately never substitutes a compaction summary: trust decisions such
 * as ticket extraction must not learn identifiers from generated summaries.
 */
export function getFirstUserMessage(branch: any[]): DialoguePart | undefined {
  for (const entry of branch) {
...
/**
 * Build a compact prompt context from Pi's resolved active entries. The caller
 * supplies SessionManager.buildContextEntries(), so messages retained before a
 * compaction entry stay available alongside the latest summary and new turns.
 */
export function getNamingContext(entries: any[], maxMessages = 6): DialoguePart[] {
  let summaryEntry: any | undefined;
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    if (entries[index]?.type === "compaction" || entries[index]?.type === "branch_summary") {
      summaryEntry = entries[index];
      break;
    }
  }
  const tail = getRecentDialogue(entries, maxMessages);
  if (!summaryEntry) return tail;
  const summary = blockText(summaryEntry.summary ?? summaryEntry.content).trim();
...
export function getInitialDialogue(entries: any[]): DialoguePart[] {
  const recent = getNamingContext(entries);
  let messageCount = 0;
  for (let index = entries.length - 1; index >= 0 && messageCount <= 2; index -= 1) {
    const role = entries[index]?.message?.role;
    if (role === "user" || role === "assistant") messageCount += 1;
  }
  if (messageCount > 2 || entries.some((entry) => entry?.type === "compaction" || entry?.type === "branch_summary")) return recent;
  const { firstUser, firstAssistant } = getFirstDialogue(entries);
