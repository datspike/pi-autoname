...
async function completeWithinBudget(model: any, prompt: string, ctx: ExtensionContext, signal: AbortSignal, remainingBudgetMs: number): Promise<any | undefined> {
  ...
  const auth = await resolveAuthWithinBudget(model, ctx, signal, deadline);
...
async function resolveAuthWithinBudget(model: any, ctx: ExtensionContext, signal: AbortSignal, deadline: number): Promise<any | undefined> {
  ...
  const auth = Promise.resolve().then(() => ctx.modelRegistry.getApiKeyAndHeaders(model));
  try {
    return await Promise.race([auth, interrupted]);
  } finally {
...
function extractDialogue(ctx: ExtensionContext, mode: NamingMode): DialoguePart[] {
  ...
  return mode === "initial" ? getInitialDialogue(entries) : getNamingContext(entries);
}
