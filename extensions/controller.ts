...
export function createNamingController(runtime: NamingControllerRuntime): NamingController {
  ...
  return {
    restore(marker, existingName) {
      ...
      if (existing) {
        // A session title without a matching extension marker may have been
        // set while this extension was inactive, restored from an older
        // version, or changed on another branch. It is user-owned unless we
        // can prove that pi-autoname wrote it.
        // Keep the pre-existing initial-naming behavior when a user explicitly
        // opts out of protection: handleSettled() will then run an initial
        // automatic name. With the default protection enabled, manualName
        // short-circuits that path before it can overwrite the title.
        state = "unnamed";
        manualName = existing;
        lastGeneratedName = existing;
        lastRenameTime = runtime.now();
        runtime.debug(`preserve existing unowned session name: ${existing}`);
        return;
      }
      state = "unnamed";
    ...
    handleSessionNameChange(name) {
      ...
      // A manual rename wins even when an automatic request is still in
      // flight. Invalidate both the request sequence and its AbortController;
      // applyResult() must not be able to write the stale AI result later.
      requestSequence += 1;
      activeRequest?.abort(new Error("Manual session name change"));
