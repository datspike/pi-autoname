...
function runtime(overrides: Partial<{
  ...
  return {
    ...
    getConfig: () => ({ enabled: true, cooldownMinutes: 10, respectManualName: overrides.respectManualName ?? true }),
    getCurrentName: () => name,
...
describe("createNamingController", () => {
  ...
  it("protects an existing name without a matching marker by default", async () => {
    const rt = runtime({ name: "Моё сохранённое имя", now: 10_000 });
    const controller = createNamingController(rt);

    controller.restore({ kind: "ai", name: "Старое AI имя", source: "ai", timestamp: 1 }, rt.name);
    await controller.handleSettled();

    expect(rt.name).toBe("Моё сохранённое имя");
    expect(rt.generateName).not.toHaveBeenCalled();
    expect(rt.markers).toEqual([]);
  });

  it("keeps the explicit opt-out behavior for an existing unowned name", async () => {
    const rt = runtime({ name: "Моё сохранённое имя", respectManualName: false, now: 10_000 });
    const controller = createNamingController(rt);

    controller.restore(undefined, rt.name);
    await controller.handleSettled();

    expect(rt.name).toBe("Автоматическое имя");
    expect(rt.generateName).toHaveBeenCalledOnce();
  });
