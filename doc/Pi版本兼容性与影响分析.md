...
2. 收集会话消息
3. 调用 LLM 生成语义化名称
4. 通过 `pi.setSessionName()` 设置名称（读取用 `pi.getSessionName()`；命名上下文用 `ctx.sessionManager.buildContextEntries()`，marker 恢复用 `ctx.sessionManager.getBranch()`）
...
| 变更项 | 影响 | 说明 |
|--------|------|------|
| Compaction summary | 🟢 已处理 | autoname 使用 Pi 的 resolved context：最新 summary、保留的 compaction 前 tail 与后续消息 |
