...
| `respectManualName` | boolean | `true` | Preserve a user-owned session title, including one set while the extension was inactive. `/autoname` remains an explicit opt-in override. |
...
When you `/name` a session, pi-autoname observes the change immediately through `session_info_changed`, persists a manual-name marker, and protects that name across future turns and session restores. An existing title without a matching autoname marker is treated as user-owned as well. To explicitly regenerate it, use `/autoname`.
...
The naming language follows natural-language text written by the user. Assistant replies, code, paths, and identifiers do not override it. CJK scripts receive explicit handling; for other scripts the model is told to preserve the user's own language rather than guessing English or Russian. If a session has no natural-language user text, Pi's own locale is used only as a fallback.

After compaction, naming uses Pi's resolved active context: the latest summary, the retained pre-compaction tail, and recent post-compaction messages. This keeps both the original task and current focus available.
